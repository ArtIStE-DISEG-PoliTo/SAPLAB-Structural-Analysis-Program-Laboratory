...
methods (Access={?sap.StructuralModel},Static)
    function coefstruct=packNonLinearSpringCoefficients(coefstruct, coefstructIndex,u0, cTol, eTol) %for non linear solver
       matObj = coefstruct.k.YoungsModulus{coefstructIndex};
       location=coefstruct.k.location{coefstructIndex};
       solution=u0(location);
       du=(solution(2,:)-solution(1,:));
       kcoeff = zeros(numel(du),1);
       for i=1:numel(du)
           [~,~,kcoeff(i)] = sap.interpCostitutiveLaw(matObj.strain, matObj.stress,'Xvalue', du(i));
       end
...
methods (Access = public)
    function tanStress=calcElasticTangentialStress(self,R)
        ...
        for i=1:numel(feistruct.regiontype)
            ...
            if isnumeric(kcoef)
               ...
               for j=1:numel(du)
                   du(j)
                   [~,F(j),K(j)] = sap.interpCostitutiveLaw(kcoef.displacement, kcoef.force,'Xvalue', du(j));
               end
            end
