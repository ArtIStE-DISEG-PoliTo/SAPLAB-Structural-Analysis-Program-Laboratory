function plot2DElementGeometry(self, g, varargin)
...
for i=1:nbs
    ...
    X=[x NaN];
    Y=[y NaN];
    plot(ax,X,Y,'LineWidth',canvas.Appearance.LineWidth,'Tag', [tag, ' - Edge ' num2str(i)],'Color', canvas.Appearance.EdgeColor,'LineStyle',canvas.Appearance.LineStyle);
    hold(ax,'on');
end
...
for i = 1:size(v,1)
    X = v(i,1);
    Y = v(i,2);
    plot(ax,X,Y,'LineWidth',canvas.Appearance.LineWidth,'Marker','o','MarkerFaceColor',canvas.Appearance.VertexColor, 'Color', canvas.Appearance.VertexColor, 'Tag', [tag, ' - Vertex ' num2str(i)], 'MarkerSize', canvas.Appearance.MarkerSize);
end

%set axis options
axis(canvas.Axes,canvas.Appearance.AxisVisibility);
canvas.Axes.Parent.Color = canvas.Appearance.BackgroundColor;

if isempty(self.Model.ParentModel.StructuralReinforcement)
    return
else
    nr = numel(self.Model.ParentModel.StructuralReinforcement.StructuralReinforcementAssignments);
    if isempty(self.Model.ParentModel.StructuralReinforcement.StructuralReinforcementAssignments)
        return
    end
    for i=1:nr
        nri=self.Model.ParentModel.StructuralReinforcement.StructuralReinforcementAssignments(i);
        nrie = nri.RegionID;
        obj  = findobj(ax,'Tag',[tag, ' - Edge ' num2str(nrie)]);
        xData = obj.XData;
        yData = obj.YData;
        plot(ax,xData,yData,'Color',canvas.Appearance.ReinforcementColor,'Tag', ['Reinforcement ' num2str(i)],'LineWidth',2,'LineJoin','chamfer');
    end
end
