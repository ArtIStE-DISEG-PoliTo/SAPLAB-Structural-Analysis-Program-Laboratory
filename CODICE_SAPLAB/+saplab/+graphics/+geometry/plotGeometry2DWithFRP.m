function plotGeometry2DWithFRP(ax, m)
%plot TwoD geometry

%input
dl = m.Geometry.geom;
appearance = m.Graphics.Appearance;
tag = m.Geometry.geomTag;
...
for i = 1:size(v,1)
    X = v(i,1);
    Y = v(i,2);
    plot(ax,X,Y,'LineWidth',appearance.LineWidth,'Marker', 'o','MarkerFaceColor',appearance.VertexColor, 'Color', appearance.VertexColor, 'Tag', [tag, ' - Vertex ' num2str(i)], 'MarkerSize', appearance.MarkerSize);
end

if ~isempty(m.StructuralReinforcement)
    nr = numel(m.StructuralReinforcement.StructuralReinforcementAssignments);  
    if isempty(m.StructuralReinforcement.StructuralReinforcementAssignments)
        return
    end
    for i=1:nr
        nri=m.StructuralReinforcement.StructuralReinforcementAssignments(i);
        nrie = nri.RegionID;
        obj  = findobj(ax,'Tag',[tag, ' - Edge ' num2str(nrie)]);
        xData = obj.XData;
        yData = obj.YData;
        plot(ax,xData,yData,'Color',appearance.ReinforcementColor,'Tag', ['Reinforcement ' num2str(i)],'LineWidth',3.5,'LineJoin','chamfer','LineStyle','-');
    end
end
