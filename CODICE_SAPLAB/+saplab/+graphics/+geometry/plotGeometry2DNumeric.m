function plotGeometry2DNumeric(ax, g, tag, appearance)
%plot TwoD geometry numeric
...
for i=1:nbs
    ...
    X=[x NaN];
    Y=[y NaN];
    plot(ax,X,Y,'LineWidth',appearance.LineWidth,'Tag', [tag, ' - Edge ' num2str(i)],'Color', appearance.NotImportedEdgeColor,'LineStyle',appearance.LineStyle);
    hold(ax,'on');
end
...
for i = 1:size(v,1)
    X = v(i,1);
    Y = v(i,2);
    plot(ax,X,Y,'LineWidth',appearance.LineWidth,'Marker', 'o','MarkerFaceColor',appearance.NotImportedVertexColor, 'Color', appearance.NotImportedVertexColor, 'Tag', [tag, ' - Vertex ' num2str(i)], 'MarkerSize', appearance.MarkerSize);
end
